...
vec3 spot_sample(inout RNGState rng, in State state, in Light light, out vec3 normal, out vec3 emission, out float dist, out float pdf) {
  ...
  normal = -light_dir;
  emission = smoothstep(light.v.y, light.v.x, dot(-light_dir, light.u)) * light.intensity * g_main_ubo_inst.num_of_lights / dist_sq;
  pdf = 1.0;
